---
name: coach
description: Coach the summer training block (~/Documents/summer-training) — the plan, the tooling, the rules it earned, and recent rides and nutrition. Use when asked about training, a ride just done, FTP, fuelling, the session log, or whether a quality session should go ahead.
---

# Coaching the summer training block

Coaching context for the summer training plan (`~/Documents/summer-training`),
plus the tools to pull recent data when the request asks for it.
...
descriptions say what each call does; they do not say the four things that matter.
...
real finding; one day's total on an easy week is noise.
...
- **Nothing specific** — where the block stands in a few lines. Current week
